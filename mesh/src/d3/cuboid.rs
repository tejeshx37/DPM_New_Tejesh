...
pub fn generate(center: Vector3<f64>, extents: Vector3<f64>, nx: u32, ny: u32, nz: u32) -> Mesh3D {
    let nx = nx.max(1) as usize;
    let ny = ny.max(1) as usize;
    let nz = nz.max(1) as usize;

    let min = center - extents * 0.5;
    let step = Vector3::new(extents.x / nx as f64, extents.y / ny as f64, extents.z / nz as f64);
